...
int main()
{
	...
	*/
	...
	// ------------------------------------
	Shader shader("stencil_testingV.txt", "stencil_testingF.txt");
	Shader shaderSingleColor("stencil_testingV-B.txt", "stencil_single_color.txt");
   ...
   // -----------------------------------------------------------------------------
	unsigned int cubeTexture = loadTexture("E:/OpenGL-/container2.png");
	unsigned int floorTexture = loadTexture("E:/OpenGL-/container2_specular.png");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		*/
		...
		// -----------------------------------------------------------------------------------------------------------------------------
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);//设置为GL_NOTEQUAL，保证只绘制箱子上模板值不为1的部分，即只绘制箱子在之前绘制的箱子之外的部分。
		glStencilMask(0x00);	//禁止模板缓冲的写入
		glDisable(GL_DEPTH_TEST);//暂时禁用深度测试，让边框不至于被地板所覆盖
		shaderSingleColor.use();
		float scale = 1.1;
		// cubes  放大一点点箱子
		glBindVertexArray(cubeVAO);
		...
		glStencilFunc(GL_ALWAYS, 0, 0xFF);//更新模板缓冲
		glEnable(GL_DEPTH_TEST);//重启深度缓冲
