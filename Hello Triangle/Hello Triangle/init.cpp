...
先定义顶点，定义完之后发送给图形渲染管线的第一个处理阶段：顶点着色器。

第一步顶点输入

GPU上会创建内存来储存这些顶点数据。
我们需要通过顶点缓冲对象VBO来管理这个内存。

第二步设置顶点缓冲对象VBO、顶点数组对象VAO
		首先绑定顶点数组对象
		然后绑定和设置对应的顶点缓冲区VBO和属性指针
		然后配置顶点属性

顶点着色器，它把一个单独的顶点作为输入。顶点着色器主要的目的是把3D坐标转为另一种3D坐标

第三步使用着色器语言，编辑顶点着色器vertex shader
...
第五步使用着色器语言，编辑片段着色器fragment shader
...
就快要完成了但还没结束，OpenGL还不知道它该如何解释内存中的顶点数据，以及它该如何将顶点数据链接到顶点着色器的属性上。我们需要告诉OpenGL怎么做。
